...
def get_features(hook, layer, inp, out):
    # print(f"Hook triggered for layer: {layer}")
...
class Hook():
    def __init__(self, model, func):
        print("CAPTAIN HOOK")
        self.hook = model.register_forward_hook(partial(func, self))
...
class CustomAttnProcessor(AttnProcessor):
    def __init__(self, attnstore, place_in_unet=None, name=None):
        super().__init__()
        fc.store_attr()
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        # print("name: ", self.name) # down_blocks.0
        key = attn.to_k(encoder_hidden_states)
        value = attn.to_v(encoder_hidden_states)
        # print("query: ", query.shape) # ->  torch.Size([1, 4096, 320])
        # print("key: ", key.shape) # -> att1: torch.Size([1, 4096, 320]) | att2: torch.Size([1, 77, 320])
        # print("value: ", value.shape) # -> att1: torch.Size([1, 4096, 320]) | att2: torch.Size([1, 77, 320])
     
        query = attn.head_to_batch_dim(query)
        key = attn.head_to_batch_dim(key)
        value = attn.head_to_batch_dim(value)
        # print("query head to batch: ", query.shape) # -> torch.Size([5, 4096, 64])
        # print("key head to batch: ", key.shape) # -> att1: torch.Size([5, 4096, 64]) | att2: torch.Size([5, 77, 64])
        # print("value head to batch: ", value.shape) # -> att1: torch.Size([5, 4096, 64]) | att2: torch.Size([5, 77, 64])
        # # heads = 5 in this case
        
        attention_probs = attn.get_attention_scores(query, key, attention_mask)
        # print("attention_probs: ", attention_probs.shape) # att1: torch.Size([5, 4096, 4096]) | att2: torch.Size([5, 4096, 77])
        # print()
