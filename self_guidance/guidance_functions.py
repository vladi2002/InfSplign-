...
from . import show_attention_maps, get_attention_map
...
def get_centroid(attn):
    ...
    # attn = threshold_attention(attn) # [8, 64, 1]
    ...
    # # TODO: redefine the weights and remove the normalization (/h and /w) in the end
    # hs = torch.linspace(0, 1, h).to(attn.device)
    # ws = torch.linspace(0, 1, w).to(attn.device)

    attn = rearrange(attn.mean(0), '(h w) d -> h w d', h=h) # taking the mean over the attention heads
    # [8, 8, 1]
    print("min: ", attn.min().item(), "| max: ", attn.max().item(), "| sum: ", attn.sum((0,1)).item())
    weighted_w = torch.sum(ws * attn, dim=[0,1])
    weighted_h = torch.sum(hs * attn, dim=[0,1])
    centroid = [weighted_w, weighted_h]
    # stack_centroid = torch.stack(centroid)
    # norm_by = h * w * attn.sum((0,1))
    # res = stack_centroid / norm_by
    centroid_x = centroid[0] / w
    centroid_y = centroid[1] / h
    res = torch.stack([centroid_x, centroid_y]) / attn.sum((0,1)) # seems like the correct normalization
    return res
...
def fix_shapes_l1(orig_attns, edit_attns, indices, tau=1, timestep=1, prompt_token_ids_map={}):
    shapes = []
...
def fix_shapes_l2(orig_attns, edit_attns, indices, tau=fc.noop, timestep=1, prompt_token_ids_map={}, object2=None):
    shapes = []
    # print(prompt_token_ids_map)
    # print(object2.item())
    if object2 is not None:
        dog = prompt_token_ids_map[object2.item()]
    for location in ["mid", "up", "down"]:
        # print(location)
        for index, o in enumerate(indices):
            # print(o)
            i = 1
            word = prompt_token_ids_map[o.item()]
            # print(word)
            deltas = []
            for edit_attn_map_integrated, ori_attn_map_integrated in zip(edit_attns[location], orig_attns[location]):
                edit_attn_map = edit_attn_map_integrated.chunk(2)[1]
                ...
                shape_orig = get_shape(orig) # torch.Size([8, 64, 1]) mid block
                target = tau(shape_orig)
                shape_edit = get_shape(edit)

                # show_attention_maps(shape_orig, target, location, 50 - timestep, i, word)
                # get_attention_map(edit, 50 - timestep, location, i, word)
                # if object2 is not None:
                #     edit_dog = edit_attn_map[:,:,object2]
                #     get_attention_map(edit_dog, 50 - timestep, location, i, dog)
                # i += 1

                # TODO: delta values are very small -> 0, 0.01, 0.02
                delta = (target - shape_edit).pow(2).mean()
                deltas.append(delta.mean())
            shapes.append(torch.stack(deltas).mean())
    return torch.stack(shapes).mean()
    ...
    for location in ["mid", "up", "down"]:
        for o in indices:
            for edit_attn_map_integrated, ori_attn_map_integrated in zip(edit_attns[location], orig_attns[location]):
                ...
                orig, edit = ori_attn_map[:,:,o], edit_attn_map[:,:,o]
                if len(orig.shape) < 3:
                    orig, edit = orig[..., None], edit[..., None]
                sizes.append(tau*get_size(orig) - get_size(edit))
    return torch.stack(sizes).mean()

def position_deltas(orig_attns, edit_attns, indices, target_centroid=None, timestep=1, prompt_token_ids_map={},):
    positions = []
        ...
        for index, o in enumerate(indices):
            ...
            if word == "balloon":
                for edit_attn_map_integrated, ori_attn_map_integrated in zip(edit_attns[location], orig_attns[location]):
                    ...
                    # orig_filename = os.path.join(attn_folder, f"orig_attention_{location}_{word}_{50 - timestep}.pt")
                    # edit_filename = os.path.join(attn_folder, f"edit_attention_{location}_{word}_{50 - timestep}.pt")
                    #
                    # torch.save(orig, orig_filename)
                    # torch.save(edit, edit_filename)

                    target = tensor(target_centroid) if target_centroid is not None else get_centroid(orig)
                    edit_centroid = get_centroid(edit) # [2, 1]
                    edit_centroid = edit_centroid.squeeze(-1) # remove the 1 dimension in the end -> [2]
                    # otherwise delta becomes [2,2] which is incorrect I think
                    delta = target.to(orig.device) - edit_centroid

                    print("edit centroid:", edit_centroid.tolist(), "| delta:", delta.tolist())

                    positions.append(delta)
    return torch.stack(positions).mean() # THIS IS THE L1 LOSS NOT THE L2 LOSS ?????
...
def move_object_by_centroid(attn_storage, indices, target_centroid=None, shape_weight=1, size_weight=1, appearance_weight=0.5, position_weight=1, ori_feats=None, edit_feats=None, timestep=1, prompt_token_ids_map={}, **kwargs):
    origs, edits = get_attns(attn_storage)
    if len(indices) > 1: 
        obj_idx, other_idx = indices
        indices = torch.cat([obj_idx, other_idx])
    shape_term = shape_weight * fix_shapes_l1(origs, edits, indices)
    print("shape_term", shape_term.item())
    # appearance_term = appearance_weight * fix_appearances(origs, ori_feats, edits, edit_feats, indices)
    appearance_term = appearance_weight * fix_appearances_by_feature(ori_feats, edit_feats, indices)
    print("appearance_term", appearance_term.item())
    size_term = size_weight * fix_sizes(origs, edits, obj_idx)
    pos_deltas = position_deltas(origs, edits, obj_idx, target_centroid=target_centroid, timestep=timestep, prompt_token_ids_map=prompt_token_ids_map)
    print("pos_deltas:", pos_deltas.item())
    position_term = position_weight * pos_deltas
    print("position_term", position_term.item())
    return shape_term + appearance_term + position_term # + size_term
# size_term = size_weight*fix_sizes(origs, edits, obj_idx)
...
def move_object_by_shape(attn_storage, indices, tau=fc.noop, shape_weight=1, appearance_weight=0.5, position_weight=1, ori_feats=None, edit_feats=None, timestep=1, prompt_token_ids_map={}, **kwargs):
    origs, edits = get_attns(attn_storage)
    ...
    appearance_term = appearance_weight * fix_appearances_by_feature(ori_feats, edit_feats, indices)
    print("appearance_term", appearance_term.item())
    move_term = position_weight * fix_shapes_l2(origs, edits, obj_idx, tau=tau, timestep=timestep, prompt_token_ids_map=prompt_token_ids_map, object2=None) # object2=other_idx
    print("move_term", move_term.item())
